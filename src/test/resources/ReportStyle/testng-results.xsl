...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="testng:suiteMethodsCount">
        ...
        <xsl:value-of
                select="if ($state = '*') then count(distinct-values($testCasesElements/class/test-method[not(@is-config)])) else count(distinct-values($testCasesElements/class/test-method[(@status=$state) and (not(@is-config))]))"/>
    </xsl:function>
    ...
    <xsl:function name="testng:testCaseMethodsCount">
        ...
        <xsl:param name="state"/>
        <xsl:value-of
                select="if ($state = '*') then count(distinct-values($testCaseElement/class/test-method[not(@is-config)])) else count(distinct-values($testCaseElement/class/test-method[(@status=$state) and (not(@is-config))]))"/>
    </xsl:function>

    <xsl:function name="testng:suiteStateClass">
        <xsl:param name="testCaseElements"/>
        <xsl:value-of select="if (count(distinct-values($testCaseElements/class/test-method[(@status = 'FAIL') and (not(@is-config))])) > 0) then 'suiteStatusFail' else 'suiteStatusPass'"/>
    </xsl:function>
                ...
                <svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0" y="0">
                 ...
                 <!--  Here need to make a change
					...
					<xsl:variable name="totalCount" select="testng:suiteMethodsCount($testCaseElements, '*')"/>
						-->					
					 <xsl:variable name="passedCount" select="count($testCaseElements/class/test-method[(@status = 'PASS') and (not(@is-config))])"/>
					 <xsl:variable name="failedCount" select="count(distinct-values($testCaseElements/class/test-method[(@status = 'FAIL') and (not(@is-config))]))"/>
					 <xsl:variable name="skippedCount" select="count($testCaseElements/class/test-method[(@status = 'SKIP') and (not(@is-config))])"/>
                    <xsl:variable name="totalCount" select="count($testCaseElements/class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])"/>
...
<!-- Here need to make a change!-->					
                                    <xsl:with-param name="failedCount"
                                                    select="count(distinct-values($testCaseElements/class/test-method[(@status = 'FAIL') and (not(@is-config))]))"/>
                                    <xsl:with-param name="passedCount"
                                                    select="count($testCaseElements/class/test-method[(@status = 'PASS') and (not(@is-config))])"/>
                                    <xsl:with-param name="skippedCount"
                                                    select="count($testCaseElements/class/test-method[(@status = 'SKIP') and (not(@is-config))])"/>
                                    <xsl:with-param name="totalCount"
                                                    select="count($testCaseElements/class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])"/>
					...
					<!--Here need to make a change	
									...
									-->
									 ...
									 <td align="center">
                                        <xsl:value-of select="count(distinct-values(./class/test-method[(@status = 'FAIL') and (not(@is-config))]))"/>
                                    </td>
                                    <td align="center">
                                    ...
                                    <td align="center">
                                        <xsl:value-of select="count(./class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])"/>
                                    </td>						
                                    <td align="center" style="font-weight: bold;">
                                        <xsl:value-of
                                                select="if (count(./class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])> 0) then format-number(count(./class/test-method[(@status = 'PASS') and (not(@is-config))]) div count(./class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))]), '###%') else '100%'"/>
                                    </td>		
                                ...
                                <tr>
                                    <td nowrap="true">
                                        ...
                                        <span style="color: red;">
                                            <xsl:value-of select="count(distinct-values($testCaseElements/class/test-method[(@status = 'FAIL') and (not(@is-config))]))"/>
                                        </span>
                                        /
                                        <span style="color: green;">
                                            <xsl:value-of select="count($testCaseElements/class/test-method[(@status = 'PASS') and (not(@is-config))])"/>
                                        </span>
                                        /
                                        <span style="color: yellow;">
                                            <xsl:value-of select="count($testCaseElements/class/test-method[(@status = 'SKIP') and (not(@is-config))])"/>
                                        </span>
                                        /
                                        <span>
                                            <xsl:value-of select="count($testCaseElements/class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])"/>
                                        </span>
                                    </td>
                                    <td style="font-weight: bold;">
                                        <xsl:value-of
                                                select="format-number(count($testCaseElements/class/test-method[(@status = 'PASS') and (not(@is-config))]) div count($testCaseElements/class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))]), '###%')"/>
                                    </td>
            ...
            <html>
                ...
                <body>
                    ...
                    <xsl:call-template name="testMethods">
                        ...
                        <xsl:with-param name="failedMethods" select="$testCaseElements/class/test-method[(@status = 'FAIL') and (not(@is-config))]"/>
                        <xsl:with-param name="passedMethods" select="$testCaseElements/class/test-method[(@status = 'PASS') and (not(@is-config))]"/>
                        <xsl:with-param name="skipedMethods" select="$testCaseElements/class/test-method[(@status = 'SKIP') and (not(@is-config))]"/>
                    </xsl:call-template>
                    <xsl:call-template name="powered-by"/>
                </body>
            </html>
    ...
    <xsl:template name="suiteTestCasesLinks">
        ...
        <xsl:for-each select="$testCases">
            ...
            <div class="testCaseLink"
                ...
                <div class="{if (count(distinct-values(./class/test-method[(@status = 'FAIL') and (not(@is-config))])) > 0)
                                then 'testCaseFail'
                                else if ((count(distinct-values(./class/test-method[(@status = 'FAIL') and (not(@is-config))])) = 0) and (count(./class/test-method[(@status = 'PASS') and (not(@is-config))]) > 0))
                                    then 'testCasePass'
                                    else 'testCaseSkip'}">
                </div>
                <xsl:value-of select="@name"/>
                    ...
                    <body>
                        <table width="100%" style="font-size: 16px; margin-bottom: 10px;" cellspacing="1">
                            <tr>
                                ...
                                <xsl:call-template name="percentageOverview">
                                    <xsl:with-param name="failedCount"  select="count(distinct-values(./class/test-method[(@status = 'FAIL') and (not(@is-config))]))"/>
                                    <xsl:with-param name="passedCount" select="count(./class/test-method[(@status = 'PASS') and (not(@is-config))])"/>
                                    <xsl:with-param name="skippedCount" select="count(./class/test-method[(@status = 'SKIP') and (not(@is-config))])"/>
                                    <xsl:with-param name="totalCount" select="count(./class/test-method[((@status = 'PASS') or (@status = 'FAIL') or (@status = 'SKIP')) and (not(@is-config))])"/>
                                    <xsl:with-param name="totalDuration"
                        ...
                        <xsl:call-template name="testMethods">
                            ...
                            <xsl:with-param name="failedMethods" select="./class/test-method[(@status = 'FAIL') and (not(@is-config))]"/>
                            <xsl:with-param name="passedMethods" select="./class/test-method[(@status = 'PASS') and (not(@is-config))]"/>
                            <xsl:with-param name="skipedMethods" select="./class/test-method[(@status = 'SKIP') and (not(@is-config))]"/>
                        </xsl:call-template>
                        <xsl:call-template name="powered-by"/>
                    </body>
                </html>
